...
// Input
// The first line contains an integer t (1≤t≤104) — the number of test cases. The first and only line of each test case contains 4 integers a1, a2, b1, b2 (1≤a1,a2,b1,b2≤10) where a1 and a2 represent the cards Suneet has, and b1 and b2 represent the cards Slavic has, respectively.
...
//Function to determine the winner in one game permutation
int determine_winner(int s1, int s2, int sl1, int sl2)
{
    int suneet_wins = 0;
    int slavic_wins = 0;
...
void solve()
{
    ...
    //Considering all permutations of the rounds
    win_count += determine_winner(a1, a2, b1, b2);
    win_count += determine_winner(a2, a1, b1, b2);
    win_count += determine_winner(a1, a2, b2, b1);
    win_count += determine_winner(a2, a1, b2, b1);

...
int calculateMinimumCost(int n, const string &s)
{
    stack<int> positions;
    int cost = 0;

    // Traverse the even indexed positions (1-based even index)
    for (int i = 1; i < n; i += 2)
    {
        if (s[i] == '(')
        {
            // Push the position (0-based index)
            positions.push(i);
        }
        else
        {
            // Pop the last unmatched opening bracket's position
            if (!positions.empty())
            {
                int openingPos = positions.top();
                positions.pop();
                cost += (i - openingPos);
            }
        }
...
int main()
{
    ...
    for (int i = 0; i < t; ++i)
    {
        ...
        string s;
        cin >> s;
        results[i] = calculateMinimumCost(n, s);
    }

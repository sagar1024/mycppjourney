...
// Input
// The first line contains a single integer t (1≤t≤104) — the number of test cases. The only line of each test case contains three integers a,b, and c (1≤a,b,c≤108) — the number of cards in the first, second, and third decks, respectively. Additional constraint on the input: a<b<c.
...
void solve()
{
    ...
    {
        int each = total / 3;
        if (each >= a && each >= b && each >= (c - (c - each))) cout << "YES" << endl;
        else cout << "NO" << endl;
    }

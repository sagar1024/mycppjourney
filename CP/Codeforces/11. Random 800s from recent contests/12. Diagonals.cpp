...
// Input
// Each test consists of several sets of input data. The first line contains a single integer t (1≤t≤500) — the number of sets of input data. Then follow the descriptions of the sets of input data. The only line of each set of input data contains two integers n, k (1≤n≤100,0≤k≤n2) — the side of the checkered board and the number of available chips, respectively.
...
void solve()
{
    ...
    //If k is 0, no diagonals are occupied
    if (k == 0)
    {
        cout << 0 << endl;
        return;
    ...
    int full_diagonals = k / n; //Full diagonals that can be occupied
    int remaining = k % n; //Remaining chips that don't fill a full diagonal

    //Minimum diagonals occupied will be full_diagonals, plus one more if there are remaining chips
    int ans = full_diagonals + (remaining > 0 ? 1 : 0);
